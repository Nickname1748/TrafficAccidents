...
def layout_html_part(all_news):
    ...
    for news in all_news:
        if type(news) == list:
            groupitems = []
            for item in news:
                groupitems.append(newsgroupitemtemplate.format(format_victims_count(item['dead'], item['injured']), item['title'], item['link'], item['article']))
            groups.append(newsgrouptemplate.format(news[0]['place'], news[0]['title'], '\n'.join(groupitems)))
        else:
            newsitems.append(newsitemtemplate.format(news['place'], format_victims_count(news['dead'], news['injured']), news['title'], news['link'], news['article']))
