...
def putindb(all_news):
	...
	for news in all_news:
		if type(news) == list:
			...
			for item in news:
				valsitem = (idnews, date, item['title'], item['article'], item['link'], item['place'], float(item['tone']['negative']), item['dead'], item['injured'], idgroup)
				cursor.execute(sqlnews, valsitem)
				idnews += 1
			idgroup += 1
		else:
			vals = (idnews, date, news['title'], news['article'], news['link'], news['place'], float(news['tone']['negative']), news['dead'], news['injured'], -1)
